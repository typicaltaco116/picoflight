 ...
 */
static void dshot_sm_config_set_pin(dshot_controller_t *controller, int pin)
{
...
void dshot_ControllerInit(dshot_controller_t *controller, uint16_t dshot_speed, PIO pio, uint8_t sm, int pin, int channels)
{
	int i;

	memset(controller, 0, sizeof(dshot_controller_t));
	controller->pio = pio;
	controller->sm = sm;
...
void dshot_RegisterTelemetryCallback(dshot_controller_t *controller, dshot_telemetry_callback_t telemetry_cb, void *context)
{
 ...
 */
static void dshot_interpret_erpm_telemetry(dshot_controller_t *controller, uint16_t edt)
{
	dshot_motor_t *motor = &controller->motor[controller->channel];
	dshot_telemetry_type_e type;
	uint16_t e, m;
	int value;
 ...
 */
static void dshot_receive(dshot_controller_t *controller, uint32_t value)
{
	int error = 0;
	uint16_t crc;
	uint32_t gcr, edt;
 ...
 */
static void dshot_cycle_channel(dshot_controller_t *controller)
{
...
void dshot_LoopAsyncStart(dshot_controller_t *controller)
{
	dshot_motor_t *motor;
...
void dshot_LoopAsyncComplete(dshot_controller_t *controller)
{
	uint32_t recv;
...
void dshot_Loop(dshot_controller_t *controller)
{
	dshot_LoopAsyncStart(controller);
	dshot_LoopAsyncComplete(controller);
}
...
void dshot_Command(dshot_controller_t *controller, uint16_t channel, uint16_t command)
{
	dshot_motor_t *motor;
...
void dshot_Throttle(dshot_controller_t *controller, uint16_t channel, uint16_t throttle)
{
	dshot_motor_t *motor;
